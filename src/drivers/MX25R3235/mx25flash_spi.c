...
#define MX25_PIN_SCLK          1
#define MX25_PORT_CS           gpioPortC
#define MX25_PIN_CS            0

#define MX25_USART             EUSART1
#define MX25_USART_CLK         cmuClock_EUSART1
...
void MX25_init( void )
{
    EUSART_TypeDef *eusart = MX25_USART;
    ...
    /* IO config */
    GPIO_PinModeSet( MX25_PORT_MOSI, MX25_PIN_MOSI, gpioModePushPull, 0 );
    ...
    GPIO->EUSARTROUTE[1].TXROUTE = ((MX25_PORT_MOSI << _GPIO_EUSART_TXROUTE_PORT_SHIFT) | (MX25_PIN_MOSI << _GPIO_EUSART_TXROUTE_PIN_SHIFT));
    GPIO->EUSARTROUTE[1].RXROUTE = ((MX25_PORT_MISO << _GPIO_EUSART_RXROUTE_PORT_SHIFT) | (MX25_PIN_MISO << _GPIO_EUSART_RXROUTE_PIN_SHIFT));
    GPIO->EUSARTROUTE[1].SCLKROUTE = ((MX25_PORT_SCLK << _GPIO_EUSART_SCLKROUTE_PORT_SHIFT) | (MX25_PIN_SCLK << _GPIO_EUSART_SCLKROUTE_PIN_SHIFT));
    GPIO->EUSARTROUTE[1].ROUTEEN = GPIO_EUSART_ROUTEEN_RXPEN | GPIO_EUSART_ROUTEEN_TXPEN | GPIO_EUSART_ROUTEEN_SCLKPEN;
...
void InsertDummyCycle( uint8_t dummy_cycle )
{
   ...
   {
      EUSART_Spi_TxRx( MX25_USART, 0xff );
   }
}
...
#ifdef SIO
   case SIO: // Single I/O
      EUSART_Spi_TxRx( MX25_USART, byte_value );
      break;
#endif
#ifdef DIO
   case DIO: // Dual I/O
...
uint8_t GetByte( uint8_t transfer_type )
{
   ...
   case SIO: // Single I/O
      ...
      data_buf = EUSART_Spi_TxRx( MX25_USART, 0xff );
      break;
#endif
#ifdef DIO
   case DIO: // Dual I/O
